...
for (j in 1:length(rhos)){
  ...
  colnames(res) <- c("gamma.tilde", "gamma.tilde2", "gamma.raw")
  ...
  for (i in 1:length(seeds)){
    ...
    res[i, 2] <- calibrated_estimator(bhat, 3, ahat.n, rho = n/N, b[3], r = r, sigma = sigma)    
    res[i, 3] <- bhat[1] - bhat[2] # \hat b1 - \hat b2
  }
  res.all[[j]] <- res
}
