...
out_NT_effect = c(0.001, 0.01, 0.1, 0.5)
...
par(mfrow = c(2,2), mar = c(4, 5, 3, 2) + 0.1, oma = c(3, 1, 0, 0) + 0.1)
for (i in 1:length(exp_NT_effect)){
  for (j in 1:length(out_NT_effect)){
    ...
    boxplot(val ~ group, main = paste0("egger: exp =", exp_NT_effect[i], "; out =", out_NT_effect[j]))
  }
}
```
...
for (i in 1:length(exp_NT_effect)){
  for (j in 1:length(out_NT_effect)){
    ...
    group = c(rep("cali", 50), rep("uncali", 50), rep("ext", 50), rep("int", 50))
    boxplot(val ~ group, main = paste0("mr.raps: exp =", exp_NT_effect[i], "; out =", out_NT_effect[j]))
  }
}
```
