...
**Step 3**: Standardize genotype. (This is an optional step, mainly for the interpretation of heritability. The variance explained by bhat vs. the noise level)

**Step 4**: Simulate $Y_1$ and $Y_2$, exposure trait and outcome trait. I set the values for beta as below for now. 
...
```{r}
# 3. standardization
T.y1 <- apply(T.y1, 2, scale)
T.y2 <- apply(T.y2, 2,scale)
NT.y1 <- apply(NT.y1, 2, scale)
NT.y2 <- apply(NT.y2, 2,scale)
